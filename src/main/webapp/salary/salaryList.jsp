...
<%
	...
	if(word == null) {
		...
		sqlSelect = "SELECT s.emp_no, CONCAT(e.first_name, ' ', e.last_name), s.salary, s.from_date, s.to_date FROM salaries s INNER JOIN employees e ON s.emp_no = e.emp_no ORDER BY s.emp_no ASC LIMIT ?,?";
	...
	// 2-2 word != null
		...
		sqlSelect = "SELECT s.emp_no, CONCAT(e.first_name, ' ', e.last_name), s.salary, s.from_date, s.to_date FROM salaries s INNER JOIN employees e ON s.emp_no = e.emp_no WHERE CONCAT(e.first_name, ' ', e.last_name) like ? ORDER BY s.emp_no ASC LIMIT ?,?";
		stmtSelect = conn.prepareStatement(sqlSelect);
		...
		<table class = "table table-hover" style = "background-color : rgb(255,255,255)">	
			<thead>					
				...
				<tr class="table-dark">
					...
					<th class = "center">연봉 계약일</th>
					<th class = "center">연봉 종료일</th>
				...
				<% 
					...
					if(currentPage > 1 && currentPage < lastPage) {
						...
						%>
							...
							<td colspan="2" class="right"><a href="<%=request.getContextPath()%>/salary/salaryList.jsp?currentPage=<%=lastPage%>"><span class="center">마지막으로</span></a></td>
						<%
						...
						%>
							...
							<td colspan="2" class="right"><a href="<%=request.getContextPath()%>/salary/salaryList.jsp?currentPage=<%=lastPage%>&word=<%=word%>"><span class="center">마지막으로</span></a></td>
						<%
						}
					}
